...
## Wnioski
 ...
 - Metoda cosine daje bardziej precyzyjne i spójne wyniki niż SVD przy niskich wymiarach. Przykładowo, dla zapytania "apache web server" poprawnie skupia się na tematach związanych z oprogramowaniem, podczas gdy SVD zwraca o wiele luźniej powiązane hasła z zapytaniem.
 - Jakość wyników SVD rośnie wraz ze wzrostem parametru k. Im większe tak, tym wyniki są bliższe tym zwracanym przez metodę cosine. Jednak powyżej k = 800 poprawa jest nieznaczna. Najczęściej jest to lekko zmieniona kolejność wyników w stosunku do k z przedziału 600-800.
 - Wydaje się, że lepiej stosować wartość parametru k z przedziału 600-800, ponieważ wtedy dostajemy hasła luźniej powiązane z zapytaniem niż przy metodzie cosine, ale nie losowe jak czasami przy małych wartościach k. Może to być przydatne przykładowo do eksploracji tematów.
